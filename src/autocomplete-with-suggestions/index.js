...
const AutocompleteWithSuggestions = ( {
	...
	help = __( 'Begin typing search term, click autocomplete result to select.', 'tawfeer-plugin' ),
	hideHelp = false, // If true, all help text will be hidden.
	label = __( 'Search', 'tawfeer-plugin' ),
	...
	postTypeLabel = __( 'item', 'tawfeer-plugin' ), // String to describe the data being shown.
	postTypeLabelPlural = __( 'items', 'tawfeer-plugin' ), // Plural string to describe the data being shown.
	...
	const classNames = [ 'tawfeer-autocomplete-with-suggestions' ];
	...
	const handleFetchSuggestions = fetchSuggestions
		...
		: async ( search = null, offset = 0, searchSlug = null ) => {
			...
			return posts.reduce( ( acc, post ) => {
				acc.push( {
					value: parseInt( post.id ),
					label: decodeEntities( post?.title.rendered ) || __( '(no title)', 'tawfeer-plugin' ),
				} );
	...
	const renderSelections = () => {
		...
		const selectedMessage = multiSelect
			? sprintf(
				...
				__( '%1$s %2$s selected', 'tawfeer-plugin' ),
				selections.length,
				selections.length > 1 ? postTypeLabelPlural : postTypeLabel
			) : sprintf(
				// Translators: %s: The label for the selection.
				__( 'Selected %s', 'tawfeer-plugin' ),
				postTypeLabel
			);

		return (
			<div className="tawfeer-autocomplete-with-suggestions__selected-items">
				<p className="tawfeer-autocomplete-with-suggestions__label">
					{ selectedMessage }
					{ 1 < selections.length && _x( ' – ', 'separator character', 'tawfeer-plugin' ) }
					{ 1 < selections.length && (
						<Button onClick={ () => onChange( [] ) } isLink isDestructive>
							{ __( 'Clear all', 'tawfeer-plugin' ) }
						</Button>
					) }
				</p>
	...
	const renderSuggestions = () => {
		...
		const className = multiSelect
			? 'tawfeer-autocomplete-with-suggestions__search-suggestions-multiselect'
			: 'tawfeer-autocomplete-with-suggestions__search-suggestions';
					...
					<p className="tawfeer-autocomplete-with-suggestions__label">
						{
							/* Translators: %s: the name of a post type. */ sprintf(
								__( 'Or, select a recent %s:', 'tawfeer-plugin' ),
								postTypeLabel
							)
						}
					</p>

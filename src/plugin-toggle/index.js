...
class PluginToggle extends Component {
	...
	retrievePluginInfo = () => {
		return new Promise( () => {
			apiFetch( { path: '/tawfeer/v1/plugins/' } ).then( pluginInfo =>
	...
	managePlugin = ( plugin, value ) => {
		...
		const params = {
			path: `/tawfeer/v1/plugins/${ plugin }/${ action }/`,
			method: 'post',
		};
						...
						this.setState( {
							pluginInfo: {
								...
								[ plugin ]: {
									...
									error:
										e.message ||
										__( 'There was an error managing this plugin.', 'tawfeer-plugin' ),
								},
							},
						} );
					} );
	...
	actionTextForPlugin = plugin => {
		...
		if ( 'configure' === inFlight ) {
			return (
				<Fragment>
					{ __( 'Installing…', 'tawfeer-plugin' ) } <Waiting isRight />
				</Fragment>
			);
		}
		if ( 'deactivate' === inFlight ) {
			return (
				<Fragment>
					{ __( 'Deactivating…', 'tawfeer-plugin' ) } <Waiting isRight />
				</Fragment>
			);
		}
		if ( ! name ) {
			return (
				<Fragment>
					{ __( 'Loading…', 'tawfeer-plugin' ) } <Waiting isRight />
				</Fragment>
			);
		}

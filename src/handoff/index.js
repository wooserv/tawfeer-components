...
class Handoff extends Component {
	...
	retrievePluginInfo = plugin => {
		...
		apiFetch( { path: '/tawfeer/v1/plugins/' + plugin } ).then( pluginInfo => {
			if ( this._isMounted ) {
				onReady( pluginInfo );
	...
	textForPlugin = pluginInfo => {
		const defaults = {
			modalBody: null,
			modalTitle: pluginInfo.Name && `${ __( 'Manage', 'tawfeer-plugin' ) } ${ pluginInfo.Name }`,
			primaryButton:
				pluginInfo.Name && `${ __( 'Manage', 'tawfeer-plugin' ) } ${ pluginInfo.Name }`,
			...
			<Fragment>
				...
				{ Name && 'active' !== Status && (
					<Button className={ classes } variant="secondary" disabled { ...otherProps }>
						{ Name + __( ' not installed', 'tawfeer-plugin' ) }
					</Button>
				) }
				{ ! Name && (
					<Button

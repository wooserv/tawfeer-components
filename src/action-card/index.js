...
class ActionCard extends Component {
	...
	render() {
		...
		const classes = classnames(
			'tawfeer-action-card',
			simple && 'tawfeer-card--is-clickable',
			hasGreyHeader && 'tawfeer-card--has-grey-header',
			hasWhiteHeader && 'tawfeer-card--has-white-header',
			hasChildren && 'tawfeer-card--has-children',
			indent && 'tawfeer-card--indent',
			isSmall && 'is-small',
			isMedium && 'is-medium',
		...
		return (
			<Card className={ classes } onClick={ simple && onClick }>
				<div className="tawfeer-action-card__region tawfeer-action-card__region-top">
					{ toggleOnChange && (
						<ToggleControl
					...
					{ image && ! toggleOnChange && (
						<div className="tawfeer-action-card__region tawfeer-action-card__region-left">
							<a href={ imageLink }>
								<div
						...
						<div className="tawfeer-action-card__region tawfeer-action-card__region-left">
							<span
								className={ classnames(
									'tawfeer-checkbox-icon',
									'is-primary',
									'checked' === checkbox && 'tawfeer-checkbox-icon--checked',
									isPending && 'tawfeer-checkbox-icon--pending'
								) }
							>
					...
					<div className="tawfeer-action-card__region tawfeer-action-card__region-center">
						<Grid columns={ 1 } gutter={ 8 } noMargin>
							<h2>
								<span className="tawfeer-action-card__title" { ...titleProps }>
								...
								{ badges?.length &&
									badges.map( ( badgeText, i ) => (
										<span key={ `badge-${ i }` } className="tawfeer-action-card__badge">
											{ badgeText }
										</span>
									) ) }
							</h2>
					...
					{ ! expandable && ( actionText || isDisplayingSecondaryAction || actionContent ) && (
						<div className="tawfeer-action-card__region tawfeer-action-card__region-right">
							...
							{ actionText &&
								( handoff ? (
									...
									<ExternalLink href={ href } className="tawfeer-action-card__primary_button">
										{ actionText }
									</ExternalLink>
								) : (
									<div className="tawfeer-action-card__container">
										{ actionText }
				...
				{ notification && (
					<div className="tawfeer-action-card__notification tawfeer-action-card__region-children">
				...
				{ children && ( ( expandable && expanded ) || ! expandable ) && (
					<div className="tawfeer-action-card__region-children">{ children }</div>
				) }
			</Card>
		);
	}

...
export default function SortableNewsletterListControl( {
	...
	return (
		<div className="tawfeer__newsletter-list-control">
			<div className="tawfeer__newsletter-list-control__selected">
				{ selected.map( selectedList => {
					...
					return (
						...
						>
							{ selected.length > 1 && (
								<span className="tawfeer__newsletter-list-control__sort-handle">
									<button
										onClick={ () => {
											const index = selected.findIndex( ( { id } ) => id === selectedList.id );
											if ( index === 0 ) {
			...
			{ getAvailableLists().length > 0 && (
				<p className="tawfeer__newsletter-list-control__lists">
					{ selected.length > 0 ? (
						<strong>{ __( 'Add more lists:', 'tawfeer-plugin' ) }</strong>
					) : (
						<strong>{ __( 'Select lists:', 'tawfeer-plugin' ) }</strong>
					) }{ ' ' }
